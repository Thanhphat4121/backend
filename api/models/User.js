...
var User = {
  ...
  setNumLoginFail: function (user, callback){
    let sql = "UPDATE user SET numLoginFail = ? WHERE user_id = ?";
    return db.query(sql,[user.numLoginFail, user.user_id], callback);
  },
  ...
  unlockUser: function(user_id,callback){
    let sql = "UPDATE user SET numLoginFail = 0 WHERE user_id = ?";
    db.query(sql,[user_id],callback);
  },
